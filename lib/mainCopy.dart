  ...
  void initState() {
    ...
    openUsbPorts(); //获取usb设备
        ...
        // }
          ...
          openUsbPorts() async {
            ...
            if(this.devices != null && this.devices.length>0) {
              for(int i=0; i<devices.length; i++) {
                dioUtil.post("遍历获取到的device, USB设备 >> ${devices[i]}, USB 名称 >> ${devices[i].productName}");
                var deviceName = devices[i].productName;
                if(deviceName.contains('Mouse')) {
                  dioUtil.post("这是一个鼠标, 跳过");
                  continue;
                }
                UsbPort _port = await devices[i].create();
                bool openResult = await _port.open();
                dioUtil.post("openResult >> $openResult");
                if (!openResult) {
                  dioUtil.post("Failed to open >> ${devices[i]}");
                }else {
                  this.usbPorts.add(_port);
                }
              }
              dioUtil.post("usbPorts >> $usbPorts");
              fetchData();
            }
          ...
          fetchData() async {
            dioUtil.post("do method fetchData...");
            dioUtil.post("fetchData.. device length: ${devices.length}, port length: ${usbPorts.length}, port detail: $usbPorts");
          ...
          sendReqData(UsbPort port) async {
            dioUtil.post("开始请求【坑位】信息...");
            await port.setDTR(true);
            await port.setRTS(true);
